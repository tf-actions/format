...
if (core.getBooleanInput("create_review", { required: true })) {
    if (context.eventName === "pull_request") {
        ...
        core.warning("Can only create a review for pull_request events. Ignoring create-review input");
    }
}
...
if (core.getInput("working_directory") !== workingDirectory) {
    ...
    if (!path.isAbsolute(userWorkingDirectory)) {
        userWorkingDirectory = path.join(process.env.GITHUB_WORKSPACE ?? "", core.getInput("working_directory"));
    }
...
const summary = core.summary
    ...
    .addList(files);
if (!createAReview) {
    summary.addRaw(`Please run \`${cliName} fmt\` locally to fix the formatting issues`, true);
}
summary.write();
